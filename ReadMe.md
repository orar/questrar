...
This will join `options.id` with `props.id` with no duplicates and
 return a `props.request` object to the component typed as
 ```js
 { data: { [id: string | number]: RequestState },
   actions: {
...
Any of `props.id` or `options.id` is optional. If no id is found, no state is returned. Thus, `request.data === {}`

* Note: If a single id is provided by either `props.id` or `options.id` or both,
 a single request state is returned instead of an object:
...
```js
import { redux } from 'questrar';

const { requestStateReducer } = redux;
...
const stateProvider = redux.createStateProvider(store, path?: string);
...
const { createStateProvider } = redux;
...
| --- | --- | --- | --- |
| `stateProvider` | `undefined` | External storage (redux, mobx) state provider which provides request states and sync request state updates with storage |
...
|`renderLoading`    | `Node` or  `(requestState) => Node` | no | Loading icon | A component that should be rendered whiles request is pending. If `renderLoading` is a component and `inject` is set, then `renderLoading` component would be injected  |
|`onFailure`        | `(requestState) => void` | no | `undefined` | A callback called everytimes a request fails |
|`renderFailure`    | `Node` or  `(requestState) => Node` | no | `undefined` | Render on request failure |
